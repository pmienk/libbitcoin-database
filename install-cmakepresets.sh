...
build_all()
{
    ...
    if [[ ! ($CI == true) ]]; then
        ...
        build_from_github_preset libbitcoin-database "builds/cmake" ${REPO_PRESET[libbitcoin-database]} "$PARALLEL" true "yes" "${BITCOIN_DATABASE_OPTIONS[@]}" $CUMULATIVE_FILTERED_ARGS_CMAKE "$@"
    else
